...
const chooserModalBackdrop = document.getElementById("chooserModalBackdrop");
const chooserModalClose = document.getElementById("chooserModalClose");
...
const scanDetailIngredients = document.getElementById("scanDetailIngredients");
const scanDetailSteps = document.getElementById("scanDetailSteps");
const scanDetailLinks = document.getElementById("scanDetailLinks");
...
function bindEvents() {
  homeSearchPrompt.addEventListener("click", openChooserModal);
  serviceNoticeClose?.addEventListener("click", clearServiceNotice);
  chooserModalBackdrop?.addEventListener("click", closeChooserModal);
  chooserModalClose?.addEventListener("click", closeChooserModal);
  ...
  document.getElementById("paymentRecipeButton").addEventListener("click", () => {
    if (state.currentRecipe) {
      navigate(state.recipeSource === "scan" ? "scan-cook" : "recipe");
    }
  });
  ...
  navItems.forEach((item) => {
    item.addEventListener("click", () => {
      if (item.dataset.route === "search") {
        openChooserModal();
        return;
      }

      closeChooserModal();
      const targetPage = item.dataset.route;
      navigate(targetPage);
    });
  });
}
...
function openChooserModal() {
  ...
  chooserModal?.setAttribute("aria-hidden", "false");
}

function closeChooserModal() {
  chooserModal?.classList.add("hidden");
  chooserModal?.setAttribute("aria-hidden", "true");
}
...
function navigate(page, options = {}) {
  ...
  if (safePage !== "home" && safePage !== "chooser") {
    closeChooserModal();
  }
...
async function requestGeminiDishPlan({ dishName, servings, allergies }) {
  const libraryRecipe = findLibraryDishPlan({ dishName, servings, allergies });
  if (libraryRecipe) {
  ...
  if (!recipe) {
    throw new Error("The demo library could not find that dish, and the live recipe service is unavailable right now.");
  }
...
async function requestGeminiIngredientSuggestions(rawIngredients) {
  const librarySuggestions = findLibraryIngredientSuggestions(rawIngredients);
  if (librarySuggestions.length) {
  ...
  if (!suggestions.length) {
    throw new Error("No usable ingredient-based recipes were found.");
  }
...
function renderIngredientSuggestions() {
  renderSuggestionCards({
    ...
    emptyMessage: "No suggestions yet. Enter your ingredients and the demo recipe library will suggest dishes.",
...
function renderScanPreview() {
  if (state.scanImageSrc) {
    ...
    if (state.scanIngredients.length) {
      const label = state.scanAnalysisSource === "gemini" ? "Live scan found" : "Library scan found";
...
async function requestOcrIngredientsFromImage() {
  ...
  const libraryScanResult = findLibraryScanSuggestions({
    imageSourceName: state.scanSourceName,
    imageDataUrl: state.scanImageSrc,
  });
  if (libraryScanResult.suggestions.length) {
  ...
  if (!suggestions.length) {
    throw new Error("No usable recipe suggestions were found for this scan.");
  }
...
function renderScanDetail() {
  if (!state.currentScanSuggestion) {
    ...
    scanDetailIngredients.innerHTML = "";
    scanDetailSteps.innerHTML = "";
    scanDetailLinks.innerHTML = "";
    return;
  }
  ...
  const ingredientMarkup = state.currentScanSuggestion.ingredients
    .map((ingredient) => `<li>${escapeHtml(ingredient.quantity)} ${escapeHtml(ingredient.unit)} ${escapeHtml(ingredient.name)}</li>`)
    .join("");
  const stepMarkup = state.currentScanSuggestion.steps
    .map((step) => `<li>${escapeHtml(step)}</li>`)
    .join("");
  const youtubeMarkup = (state.currentScanSuggestion.youtubeLinks || [])
    .map((link) => `<a href="${safeUrl(link)}" target="_blank" rel="noreferrer">Open cooking guide</a>`)
    .join("");
  ...
  scanDetailDescription.innerHTML = `
    ...
    <p>Detected ingredients: ${escapeHtml(state.scanIngredients.join(", ") || "demo pantry match")}.</p>
  `;
  scanDetailIngredients.innerHTML = ingredientMarkup;
  scanDetailSteps.innerHTML = stepMarkup;
  scanDetailLinks.innerHTML = [youtubeMarkup, sourceMarkup].filter(Boolean).join("");
}
...
function renderScanCook() {
  ...
  const detectedLabel = state.scanAnalysisSource === "gemini"
    ? "Detected by live scan analysis"
    : "Matched from the demo grocery library";
...
function getDemoRecipes() {
  return Array.isArray(window.NAU_DEMO_LIBRARY?.recipes) ? window.NAU_DEMO_LIBRARY.recipes : [];
}

function findLibraryDishPlan({ dishName, servings, allergies }) {
  const recipe = getBestLibraryDishMatch(dishName);
  if (!recipe) {
    return null;
  }

  return buildLibraryRecipePayload(recipe, {
    servings,
    allergies,
  });
}

function findLibraryIngredientSuggestions(rawIngredients) {
  const requestedKeywords = extractKeywords(rawIngredients);
  ...
  return getDemoRecipes()
    .map((recipe) => ({
      recipe,
      score: scoreRecipeAgainstKeywords(recipe, requestedKeywords),
      matched: getMatchedRecipeKeywords(recipe, requestedKeywords),
    }))
    .filter((entry) => entry.score > 0)
    .sort((left, right) => right.score - left.score)
    .slice(0, 4)
    .map((entry) => buildLibrarySuggestion(entry.recipe, {
      matchedKeywords: entry.matched,
      contextLabel: `A good match for ${entry.matched.join(", ")}.`,
    }));
}

function findLibraryScanSuggestions({ imageSourceName }) {
  const sourceKeywords = extractKeywords(imageSourceName);
  let rankedRecipes = getDemoRecipes()
    .map((recipe) => ({
      recipe,
      score: scoreRecipeAgainstKeywords(recipe, sourceKeywords),
      matched: getMatchedRecipeKeywords(recipe, sourceKeywords),
    }))
    .filter((entry) => entry.score > 0)
    .sort((left, right) => right.score - left.score)
    .slice(0, 4);
  ...
  if (!rankedRecipes.length) {
    const fallbackIds = ["com-chien-duong-chau", "trung-chien-ca-chua", "rau-cu-xao-nam", "thit-kho-trung"];
    rankedRecipes = fallbackIds
      .map((id) => getDemoRecipes().find((recipe) => recipe.id === id))
      .filter(Boolean)
      .map((recipe, index) => ({
        recipe,
        score: fallbackIds.length - index,
        matched: [],
      }));
  }

  const suggestions = rankedRecipes.map((entry) => buildLibrarySuggestion(entry.recipe, {
    matchedKeywords: entry.matched,
    contextLabel: entry.matched.length
      ? `Matched from your scan context: ${entry.matched.join(", ")}.`
      : "Demo scan mode suggested flexible dishes that usually work well with mixed groceries.",
  }));
  const ingredients = [...new Set(
    suggestions
      .flatMap((suggestion) => suggestion.ingredients.slice(0, 3).map((ingredient) => ingredient.name))
      .slice(0, 8)
  )];
...
function getBestLibraryDishMatch(dishName) {
  const query = normalizeSearchText(dishName);
  if (!query) {
    return null;
  }
  ...
  return getDemoRecipes()
    .map((recipe) => ({
      recipe,
      score: scoreDishNameMatch(recipe, query),
    }))
    .filter((entry) => entry.score > 0)
    .sort((left, right) => right.score - left.score)[0]?.recipe || null;
}

function buildLibraryRecipePayload(recipe, options = {}) {
  const serves = Math.max(1, Number(options.servings) || recipe.serves || 2);
  const scaledIngredients = scaleLibraryIngredients(recipe.ingredients, recipe.serves || serves, serves);
  const allergyWarnings = [
    ...new Set(findAllergyWarnings(scaledIngredients, options.allergies || [])),
  ];
  ...
  return {
    ...
    steps: normalizeInstructionSteps(recipe.steps),
    youtubeLinks: normalizeRecipeLinks([], recipe.title),
    sourceLinks: [],
    allergyWarnings,
  };
}

function buildLibrarySuggestion(recipe, options = {}) {
  const matchedKeywords = Array.isArray(options.matchedKeywords) ? options.matchedKeywords : [];
  const descriptionParts = [
    recipe.description,
    options.contextLabel || "",
    matchedKeywords.length ? `This dish uses ${matchedKeywords.join(", ")} well.` : "",
  ].filter(Boolean);
  ...
  return {
    ...
    ingredients: scaleLibraryIngredients(recipe.ingredients, recipe.serves || 2, recipe.serves || 2),
    steps: normalizeInstructionSteps(recipe.steps),
    youtubeLinks: normalizeRecipeLinks([], recipe.title),
    sourceLinks: [],
    extras: cloneExtras(recipe.extras || []),
    image: recipe.image || getImageForCategory(inferCategory(recipe.title)),
    heroImage: recipe.heroImage || recipe.image || getImageForCategory(inferCategory(recipe.title)),
  };
}

function scoreDishNameMatch(recipe, query) {
  const names = [recipe.title, ...(Array.isArray(recipe.aliases) ? recipe.aliases : [])]
  ...
  names.forEach((name) => {
    ...
    const sharedTokens = queryTokens.filter((token) => name.includes(token)).length;
    score = Math.max(score, sharedTokens * 12);
  });

  return score;
}

function scoreRecipeAgainstKeywords(recipe, keywords) {
  const haystack = getRecipeSearchBlob(recipe);
...
function getMatchedRecipeKeywords(recipe, keywords) {
  const haystack = getRecipeSearchBlob(recipe);
  return [...new Set(keywords.filter((keyword) => keyword && haystack.includes(keyword)))].slice(0, 5);
}
...
function getRecipeSearchBlob(recipe) {
  return normalizeSearchText([
    ...
    ...(Array.isArray(recipe.matchKeywords) ? recipe.matchKeywords : []),
    ...(Array.isArray(recipe.ingredients) ? recipe.ingredients.map((ingredient) => ingredient.name) : []),
  ].join(" "));
}

function scaleLibraryIngredients(ingredients, baseServes, targetServes) {
  const safeBase = Math.max(1, Number(baseServes) || 1);
  const safeTarget = Math.max(1, Number(targetServes) || safeBase);
  const factor = safeTarget / safeBase;

  return (Array.isArray(ingredients) ? ingredients : [])
    .map((ingredient) => ({
      ...ingredient,
      quantity: scaleNumericValue(ingredient.quantity, factor),
...
function extractKeywords(value) {
  const segments = normalizeList(value).map(normalizeSearchText).filter(Boolean);
  const tokens = segments.flatMap((segment) => segment.split(" ").filter((token) => token.length >= 3));
  return [...new Set([...segments, ...tokens])];
}
...
function buildCartFromRecipe(recipe, allergies) {
  return recipe.ingredients.map((ingredient) => ({
    ...
    ...ingredient,
    flagged: allergies.some((allergy) => normalizeSearchText(ingredient.name).includes(normalizeSearchText(allergy))),
    image: ingredient.image || getImageForCategory(ingredient.category),
  }));
}
...
function renderCart() {
  ...
  if (!state.cart.length) {
    cartList.innerHTML = '<div class="cart-alert">Your dish ingredients will appear here after the recipe library prepares your cart.</div>';
    allergyAlertBox.classList.add("hidden");
    return;
  }
...
function findAllergyWarnings(ingredients, allergies) {
  return ingredients
    .filter((ingredient) => allergies.some((allergy) => normalizeSearchText(ingredient.name).includes(normalizeSearchText(allergy))))
...
function inferCategory(value) {
  ...
  if (hasAny(text, ["fish", "salmon", "catfish", "mackerel", "ca", "tom"])) return "fish";
  if (hasAny(text, ["beef", "pork", "meat", "thit", "heo", "bo", "sausage", "ham"])) return "meat";
  if (hasAny(text, ["milk", "cheese", "yogurt", "sua", "butter", "tofu"])) return "dairy";
  if (hasAny(text, ["egg", "trung"])) return "egg";
  if (hasAny(text, ["rice", "com", "gao", "bun", "bread", "noodle", "mi", "pasta"])) return "pantry";
  return "vegetable";
}
